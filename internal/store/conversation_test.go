...
func TestIsResultProcessed(t *testing.T) {
	s := setupTestStore(t)

	// Enqueue an acked result
	if err := s.Enqueue(&QueuedMessage{
		Type:      "result",
	...
	processed, err := s.IsResultProcessed("backend-dev", "task-001")
	if err != nil {
		t.Fatalf("IsResultProcessed failed: %v", err)
	}
	if !processed {
		t.Error("expected processed=true for acked result")
	}
}

func TestIsResultProcessed_NotFound(t *testing.T) {
	s := setupTestStore(t)

	processed, err := s.IsResultProcessed("nonexistent", "task-999")
	if err != nil {
		t.Fatalf("IsResultProcessed failed: %v", err)
	}
	if processed {
		t.Error("expected processed=false for nonexistent")
	}
}

func TestIsResultProcessed_QueuedNotProcessed(t *testing.T) {
	s := setupTestStore(t)

	// Enqueue a result that is still queued (not acked)
	if err := s.Enqueue(&QueuedMessage{
		Type:      "result",
		FromAgent: "backend-dev",
		ToAgent:   "orchestrator",
		Body:      `{"task_id":"task-queued"}`,
		Status:    "queued",
	}); err != nil {
		t.Fatalf("Enqueue failed: %v", err)
	}

	processed, err := s.IsResultProcessed("backend-dev", "task-queued")
	if err != nil {
		t.Fatalf("IsResultProcessed failed: %v", err)
	}
	if processed {
		t.Error("expected processed=false for queued (non-acked) result")
	}
}

func TestIsResultProcessed_ContextField(t *testing.T) {
	s := setupTestStore(t)

	// Enqueue with task_id in context (not body)
	if err := s.Enqueue(&QueuedMessage{
		Type:      "result",
		FromAgent: "architect",
		ToAgent:   "orchestrator",
		Body:      `{"status":"completed"}`,
		Context:   `{"task_id":"task-ctx-001"}`,
		Status:    "acked",
	}); err != nil {
		t.Fatalf("Enqueue failed: %v", err)
	}

	processed, err := s.IsResultProcessed("architect", "task-ctx-001")
	if err != nil {
		t.Fatalf("IsResultProcessed failed: %v", err)
	}
	if !processed {
		t.Error("expected processed=true for acked result matched via context field")
	}
}

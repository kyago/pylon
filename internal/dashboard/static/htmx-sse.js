...
(function() {
    ...
    onEvent: function(name, evt) {
      ...
      switch (name) {
        case 'htmx:beforeCleanupElement':
          ...
          if (source) {
            api.triggerEvent(parent, 'htmx:sseClose', { source, type: 'nodeReplaced' })
  ...
  function registerSSE(elt) {
    if (api.getAttributeValue(elt, 'sse-swap')) {
      ...
      for (var i = 0; i < sseEventNames.length; i++) {
        ...
        const listener = function(event) {
          if (maybeCloseSSESource(sourceElement)) return
          if (!api.bodyContains(elt)) {
            source.removeEventListener(sseEventName, listener)
            return
          }
          if (!api.triggerEvent(elt, 'htmx:sseBeforeMessage', event)) return
          swap(elt, event.data)
    ...
    if (api.getAttributeValue(elt, 'hx-trigger')) {
      ...
      triggerSpecs.forEach(function(ts) {
        if (ts.trigger.slice(0, 4) !== 'sse:') return
  ...
  function ensureEventSource(elt, url, retryCount) {
    ...
    if (closeAttribute) {
      source.addEventListener(closeAttribute, function() {
        api.triggerEvent(elt, 'htmx:sseClose', { source, type: 'message' })
        source.close()
      })
    }
  }
  ...
  function maybeCloseSSESource(elt) {
    if (!api.bodyContains(elt)) {
      ...
      if (source != undefined) {
        api.triggerEvent(elt, 'htmx:sseClose', { source, type: 'nodeMissing' })
        source.close()
        return true
      }
    }

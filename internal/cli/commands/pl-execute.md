...
4. 각 에이전트 완료 후 검증 출력을 확인한 뒤 `execution-log.json`에 기록합니다.
   검증 출력이 첨부되지 않은 "완료" 보고는 완료로 처리하지 않습니다. 또한 provider의 자연어 응답을
   정본으로 사용하지 않고, task별 구조화 보고서를 Pylon으로 검증해 기록합니다:

```bash
TASK_REPORT_INPUT="$PIPELINE_DIR/tasks/$TASK_ID/task-report-input.json"
TASK_REPORT="$PIPELINE_DIR/tasks/$TASK_ID/task-report.json"
mkdir -p "$PIPELINE_DIR/tasks/$TASK_ID"
...
cat > "$TASK_REPORT_INPUT" <<JSON
{
  ...
  "evidence_refs": ["tasks/$TASK_ID/test-output.log"],
  "hypotheses_rejected": [
...
태스크의 최종 성공 여부는 이 보고서가 아니라 deterministic gate와 격리 evaluator가 모두 끝난 뒤 상위
pipeline이 `execution-log.json`의 task 항목에 `verified: true|false`로 기록합니다.

`task-report.json`은 create-once이며 digest가 포함됩니다. 재시도는 `attempt`를 올려 새 파일
(`task-report-<attempt>.json`)로 기록하고 기존 파일을 덮어쓰지 않습니다.

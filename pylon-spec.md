...
- **워크스페이스**: 프로젝트들의 상위 디렉토리. 팀 도메인 지식과 프로젝트 클론들이 위치한다. git 추적은 선택사항 (기본은 git 미사용).
- **프로젝트**: 워크스페이스 하위에 clone된 독립적인 코드 저장소
...
```
workspace/                              ← 프로젝트들의 상위 디렉토리 (git 선택)
├── .pylon/
...
- 결과: `.pylon/` 디렉토리 + `.gitignore` 설정

#### `pylon add-project [git-url]`

- git clone으로 워크스페이스에 프로젝트 추가 (`.gitmodules` 미생성)
...
- 결과: 워크스페이스 내 독립 clone 디렉토리 + 프로젝트별 `.pylon/` 생성

#### `pylon migrate-project <name>`

- 기존 git submodule 방식의 프로젝트를 standalone clone으로 전환
- **플래그**: `--dry-run` (안전 검사 결과 출력, 상태 변경 없음), `--force` (안전 검사 무시)
- 안전 검사: dirty tree, unpushed commits, local-only 브랜치, pin SHA 불일치 시 블록
- `--force` 없이는 데이터 손실 가능한 상황을 차단
...
- **플래그**: `--force` (확인 생략), `--dry-run` (삭제 예정 항목만 표시), `--remove-projects` (프로젝트 디렉토리도 제거), `--remove-binary` (바이너리도 제거)

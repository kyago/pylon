...
│   │   │   └── {agent-name}/
│   │   │       └── {task-id}.result.json
│   │   ├── state.json                  ← 파이프라인 상태 (오케스트레이터 관리)
...
- 중단된 대화 재개
- 기존 대화 컨텍스트를 로드하여 PO 에이전트와 대화 계속
...
- 웹 대시보드 로컬 서버 실행 (Templ + HTMX)
- **CLI 동등 기능**: request, cancel, resume 등 모든 CLI 기능을 웹에서도 수행 가능
- 에이전트 실시간 상태 (SSE 스트리밍)
- 태스크 진행률, 로그 모니터링
- 대화 히스토리 조회
...
learnings 필드 존재 시 → project_memory INSERT
    ↓
state.json 갱신 + pipeline_state UPDATE
    ↓
토픽 매칭 → 구독 에이전트에게 알림
    ↓
블랙보드 갱신 (결정/결과 기록)
...
└── 20260305-143022-user-login/
    ├── thread.md       ← 전체 대화 기록 (사람 ↔ PO)
    └── meta.yml        ← 상태, 시작시간, 관련 프로젝트, task_id

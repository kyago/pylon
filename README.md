...
사용자 ←→ Claude Code TUI (루트 에이전트 / PO)
              ...
              │
              └── 서브 에이전트 (Claude Code Agent 도구, 23종)
                   ├── architect ─── 기술 분석, 아키텍처 결정
                   ├── pm ────────── 태스크 분해, 조율
                   ├── backend-dev ─ 백엔드 구현
                   ├── frontend-dev  프론트엔드 구현
                   ├── test-engineer 테스트 작성
                   ├── tech-writer ─ 도메인 문서 갱신
                   └── ... (debugger, designer, devops 등 17종)
```

Pylon은 Go CLI가 워크스페이스를 준비한 뒤, Claude Code를 직접 실행(`syscall.Exec`)합니다. 서브 에이전트는 Claude Code 네이티브 기능(Agent tool, TeamCreate)으로 생성되어 별도의 프로세스 관리 계층 없이 동작합니다.
...
`pylon init` 시 `.pylon/agents/`에 설치됩니다. `pylon sync-agents`로 최신 버전으로 갱신할 수 있습니다.

| 에이전트 | 역할 |
|----------|------|
| po | 프로덕트 오너 (요구사항 분석, 사용자 대화) |
...
| `pylon request <requirement>` | AI 에이전트 팀에 요구사항 제출 |
| `pylon cancel [pipeline-id]` | 진행 중인 파이프라인 취소 |
| `pylon resume [conversation-id]` | 중단된 세션 재개 |
| `pylon review [pr-url]` | PR 코드 리뷰 |
| `pylon dashboard` | 웹 대시보드 실행 (기본 localhost:7777) |

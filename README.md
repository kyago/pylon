...
| **Fossil** | Pylon 작업 이력 | `brew install fossil` / https://fossil-scm.org/home/uv/download.html |
| **git** | 버전 관리 | 기본 설치 |
...
> **기존 워크스페이스 마이그레이션:** 업데이트 후 Fossil을 설치하고 `pylon doctor`를
> 실행해 설정 기본값을 동기화하세요. 작업 이력 저장소는 첫 `pylon` 세션 실행 시 자동으로
> 초기화됩니다. 세션 실행 전에 미리 준비하려면 `pylon history init`을 실행할 수 있습니다.
...
| `pylon doctor` | 필수 도구(fossil/git/gh/claude) 확인 + 워크스페이스 리소스·설정 동기화 (`--fix-excludes`) |
| `pylon version` | 버전 정보 |
...
Pylon은 프로젝트의 Git 저장소와 분리된 Fossil shadow checkout에 선별된 작업 이력을 기록합니다.
파이프라인마다 계획 완료, 실행 완료, 최종 완료의 세 체크포인트가 생성됩니다.

```bash
pylon history init
pylon history log --pipeline <pipeline-id>
pylon history show <checkin>
pylon history diff <from> <to>
pylon history sync
pylon history export <checkin> --output <new-directory>
```
...
│   ├── history/               # Fossil 저장소 + shadow checkout (git 무시)
│   │   ├── pylon-history.fossil
│   │   ├── checkout/
│   │   └── state.json
...
history:
  remote: ""                 # 선택적 Fossil 원격 URL
  sync_on_checkpoint: true   # remote 설정 시 체크포인트 후 동기화
...
| 메모리 검색 | FTS5 BM25 |
| 작업 이력 | Fossil shadow repository |

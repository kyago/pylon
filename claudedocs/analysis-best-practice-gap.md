...
**불필요 이유**: Pylon은 워크스페이스 + 독립 git clone 구조이지 모노레포가 아니다. 각 프로젝트는 독립된 git repo이므로 CLAUDE.md 계층적 로딩 전략이 적용되지 않는다. 대신 오케스트레이터가 에이전트 실행 시 필요한 컨텍스트를 명시적으로 주입하는 방식을 사용한다.
